...
sub_image2 = rospy.Subscriber("/camera/color/image_raw", Image)   # making "global" no callback
...
def get_depth(depth_msg):
  ...
  depth = cv_image[369, 129]    # hard coding pixel coords to test
  #x, y = image_callback(sub_image2)
  rospy.loginfo("The coordinates of the object are " +str(cv_image))
...
def main():
  ...
  sub_image = rospy.Subscriber("/camera/color/image_raw", Image, callback=image_callback)
  sub_depth = rospy.Subscriber("/camera/depth/image_raw", Image, callback=get_depth)
